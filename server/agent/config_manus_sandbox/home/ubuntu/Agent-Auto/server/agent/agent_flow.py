...
- LLM:             Cerebras AI (via https://cloud.cerebras.ai/) with native tool calling
...
- Model:           qwen-3-235b-a22b-instruct-2507 (verified native tool calling)
"""
import os
import re
import sys
...
if not CEREBRAS_API_KEY:
    sys.stderr.write("[agent] WARNING: CEREBRAS_API_KEY is not set!\n")
    sys.stderr.flush()


def _build_tool_schemas() -> List[Dict[str, Any]]:
    """Build TOOL_SCHEMAS dynamically from class-based tool registry.
    Adds the special 'idle' tool and converts OpenAI format to Cerebras-compatible format.
    """
    idle_schema = {
        "name": "idle",
        "description": (
...
def call_cerebras_streaming(messages: list) -> str:
    """Synchronous Cerebras streaming call. Returns full text."""
    url = CEREBRAS_API_URL
    body: Dict[str, Any] = {"model": CEREBRAS_MODEL, "messages": messages, "max_tokens": 4096, "stream": True}
    ...
    try:
        ...
        sys.stderr.write("Cerebras streaming error: {}\n".format(e))
        sys.stderr.flush()
    if full_text:
        t = full_text.strip()
...
async def call_cerebras_streaming_realtime(
    ...
    """
    True async streaming from Cerebras AI.
    ...
    def _stream_worker() -> None:
        ...
        try:
            ...
            sys.stderr.write("Cerebras realtime streaming error: {}\n".format(e))
            sys.stderr.flush()
        loop.call_soon_threadsafe(queue.put_nowait, None)
...
def call_cerebras_api(
    ...
    url = CEREBRAS_API_URL
    body: Dict[str, Any] = {"model": CEREBRAS_MODEL, "messages": messages, "max_tokens": 4096, "stream": False}
    if tools:
        body["tools"] = tools
    data = json.dumps(body).encode("utf-8")
    req = urllib.request.Request(
    ...
    if "success" in result and not result["success"]:
        ...
        raise urllib.error.HTTPError(url, 500, "Cerebras API error: {}".format(errors), {}, None)
    return result


def _normalize_response_text(value: Any) -> str:
    """Normalize a Cerebras 'response' field to always be a string.
    
    Some models return structured JSON objects directly
...
def call_api_with_retry(
    ...
    """Call Cerebras API with retry. Falls back to text-only mode if tools cause 400."""
...
def _extract_cerebras_response(api_result: Dict[str, Any]) -> tuple:
    """Extract (text, tool_calls) from Cerebras response."""
    parsed = api_result.get("result", api_result)
    if not isinstance(parsed, dict):
        parsed = api_result
    raw_text = parsed.get("response")
    text = _normalize_response_text(raw_text)
...
class DzeckAgent:
    ...
    async def _pre_plan_clarification_check(
        ...
        try:
            loop = asyncio.get_event_loop()
            url = CEREBRAS_API_URL
            body = {"model": CEREBRAS_MODEL, "messages": messages, "max_tokens": 200, "stream": False}
            data = json.dumps(body).encode("utf-8")
            req = urllib.request.Request(
                url, data=data,
                headers={
