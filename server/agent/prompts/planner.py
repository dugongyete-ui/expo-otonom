...
- Script/kode kerja → ~/ atau $HOME (workspace, tidak muncul download). Gunakan ~ atau $HOME, jangan hardcode path home.
- File HASIL untuk user → ~/output/ (muncul tombol download)

ATURAN:
- Text files: langkah pakai file_write ke ~/output/namafile.ext → Files Agent
...
PACKAGE MANAGEMENT:
- pip: gunakan `pip3 install <pkg>` atau `python3 -m pip install <pkg>` (tanpa flag tambahan di E2B sandbox)
- npm: bekerja normal
